...
{/* TWO IMPORTS ARE DELIBERATELY ABSENT, and neither is an oversight.

    FanOut — unspecified on purpose. Which teams and which touchpoints were never
    pinned down, so section 05 ships without a module rather than with an invented
    org chart. Build it when the contents are decided, not before.
...
{/* Section 05 ships WITHOUT a module, deliberately. FanOut was specified only as a
    name: which teams and which touchpoints were never pinned down, and an invented
    org chart would be a worse answer than an absent one. The section's prose already
    carries the argument. Restore <FanOut /> here when its contents are decided.

    FLAGGED FOR PAUL: this is the one section of the seven with no visual. */}
